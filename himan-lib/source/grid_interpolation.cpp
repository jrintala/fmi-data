...
interpolation_kernel::interpolation_kernel(latitude_longitude_grid* target, latitude_longitude_grid* source)
{
	...
	for (size_t i = 0; i < target->Size(); ++i)
	{
		...
		kernel.row_indices[4*i  ]=i; kernel.column_indices[4*i  ]=a; kernel.values[4*i  ]=w[0];
		kernel.row_indices[4*i+1]=i; kernel.column_indices[4*i+1]=b; kernel.values[4*i+1]=w[1];
		kernel.row_indices[4*i+2]=i; kernel.column_indices[4*i+2]=c; kernel.values[4*i+2]=w[2];
		kernel.row_indices[4*i+3]=i; kernel.column_indices[4*i+3]=d; kernel.values[4*i+3]=w[3];
	}
}

interpolation_kernel::interpolation_kernel(rotated_latitude_longitude_grid* target, latitude_longitude_grid* source) : interpolation_kernel(dynamic_cast<latitude_longitude_grid*>(target), source) {}

#ifdef DEBUG
void check_geometry(const himan::point& a, const himan::point& b, const himan::point& c, const himan::point& d, const himan::point& p)
{
	...
	if(!boost::geometry::equals(poly, hull)) 
		throw std::runtime_error("Grid points do not form convex hull");
