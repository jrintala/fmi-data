...
void himan::grid_interpolation::interpolate(std::vector<double>& target, const std::vector<double>& source,
                                            const himan::grid_interpolation::interpolation_kernel& kernel)
{
...
std::vector<double> himan::grid_interpolation::get_weights(const himan::point& a, const himan::point& b,
                                                           const himan::point& c, const himan::point& d,
                                                           const himan::point& p)
{
	...
	if (a.X() == c.X() && a.Y() == b.Y() && d.X() == b.X() && d.Y() == c.Y())
	{
		 ...
		 *                                          |    0     0     1     1|
		 *                                          |    0     0    y0    y1|
		             * with C0, C1, C2, C3 to solve for. In the following code segment this is done through manually
		 * inverting the 4x4 matrix first and then
		...
		/*
		             * Apply coordinate transformation to transform point p from latlon space to point phi in unit
		 * square.
		             *
		             *                               |  1   0|
